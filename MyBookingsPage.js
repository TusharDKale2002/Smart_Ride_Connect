...
const token = localStorage.getItem('token');
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await fetch(`https://localhost:44351/api/Booking/my-bookings/${userId}`, {
        headers: {
      ...
      const transformedBookings = data.map(booking => ({
        ...
        date: new Date().toISOString().split('T')[0], // Default date since it's not in the response
        time: booking.departureTime,
        driverName: booking.passengerName, // This will be the passenger name, not driver
        carModel: '5-seater', // Default since not in response
        price: 0, // Default since not in response
  ...
  fetchBookings();
    ...
    const handleCancelBooking = useCallback(async (id) => {
        try {
            ...
            const response = await fetch('https://localhost:44351/api/Booking/update-request', {
                method: 'PUT',
                headers: {
                                            ...
                                            <div className="list-group">
                                                {filteredBookings.map(booking => (
                                                    <div key={booking.id} className="list-group-item list-group-item-action">
                                                        ...
                                                        <small>{booking.date} at {booking.time} | ${booking.price}</small>
                                                        <div className="mt-2">
                                                            <button
                                                            ...
                                                            >
                                                                View Details
                                                                                                                         ...
                                                                                                                         {tab === 'upcoming' && booking.status === 'pending' && (
                                                                 <button
